...
class Buffer:
    ...
    def __init__(self, obs_dim, size, num_envs, device, gamma=0.99, gae_lambda=0.95):
        # Initialize buffer
        self.capacity = size
        self.obs_buf = torch.zeros((size, num_envs, *obs_dim), dtype=torch.uint8, device=device)
        self.act_buf = torch.zeros((size, num_envs), dtype=torch.int64, device=device)
    ...
    def get(self):
        ...
        self.ptr = 0
        return self.obs_buf, self.act_buf, self.val_buf, self.logprob_buf, self.term_buf, self.trunc_buf
